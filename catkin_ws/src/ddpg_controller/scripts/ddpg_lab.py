...
# Callback for switching ddpg on/off during testing on lab. If swith=True, we use ddpg. If False, use manual control
use_ddpg = True
def ctr_switch_callback(data):
    global use_ddpg
    use_ddpg = data.data
...
class DroneState():
    ...
    def __init__(self):
        ...
        self.state_size = 12 # x y z vx vy vz ax ay az a_prev
    ...
    def step(self, state_reduced, action):
        ...
        reward, done, pos_error = self.get_reward(state_reduced, action, next_state)
    ...
    def get_reward(self, state_reduced, current_action, next_state):
        reward = 0.0
        ...
        previous_action = np.array(state_reduced[0][-3:])
        print("current_action", current_action)
        print("previous_action", previous_action)

        # factor_delta_action = 0.05 * 0.8/2.0
        factor_delta_action = 0.02 * 0
        delta_action = current_action - previous_action

        # print("delta_action", delta_action)

        #factor_action = 0.02 * 0.4
        factor_action = 0.08
        ...
        print("aaaaaaaaaaaaa", factor_delta_action * np.linalg.norm(delta_action))

        #reward = np.exp( - pos_error / (2 * sigma **2) )
        if pos_error < 0.5:
            reward = np.exp( - pos_error / (2 * sigma **2) ) - \
                factor_action * np.linalg.norm(current_action) - \
                    factor_delta_action * np.linalg.norm(delta_action)
        else:
            reward = np.exp( - pos_error / (2 * sigma **2) )            
...
def init_subscribers(deploy_in_lab):
    if deploy_in_lab:
    ...
    rospy.Subscriber('/use_ddpg', Bool, ctr_switch_callback)
...
if train_agent == False:
    actor_imported = tf.keras.models.load_model('/home/danie/catkin_ws/src/ddpg/src/results/apr20_3/actor_model.hdf5')

max_episodes = 10000 # number of episodes to train drone in
max_total_steps = 50000 # max number of steps in total for the whole training period
max_steps = 50 # max number of steps in each training episode

init_subscribers(deploy_in_lab)
...
def run_episode(max_steps, training):
    ...
    steps_completed = 0 
    if not deploy_in_lab: # if in lab we use manual control to start drone
        state = env.reset()
        takeoff()
    ...
    if training:
        ...
        for step_number in range(max_steps):
            ...
            # build state vector
            state_reduced = env.reduce_state_array(state) # reduce state space to x y z vx vy vz a_prev
            ...
            # if episode is done and it only contained 1 step (or even 0, if drone is initialized outside valid area), 
            # then we do not store it in RB
            # if done == True:
            #     if step_number <= 1:
            #         print("episode done on first step. not adding to RB")
            #     else:
    ...
    # rate.sleep()
        ...
        time_start_of_episode = rospy.get_rostime()
        time_start_of_no_estimate = rospy.get_rostime()
        now = rospy.get_rostime()
        state = env.get_drone_state()
        prev_drone_estimate_status = 0
        curr_drone_estimate_status = 0
        prev_pos_error = 100
        # while True:
        # while (now.secs - time_start_of_episode.secs) <= 30 and use_ddpg:
        if use_ddpg: 
            for step_number in range(max_steps):
                now = rospy.get_rostime()

                # handle case of losing Thomas' estimate: stopping ddpg and enabling manual keyboard control
                # prev_drone_estimate_status = curr_drone_estimate_status
                # curr_drone_estimate_status = env.get_drone_estimate_status
                # if prev_drone_estimate_status != 0 and curr_drone_estimate_status == 0: # gone from having an estimate to not having one
                #     time_start_of_no_estimate = rospy.get_rostime() # make a timestamp
                #     no_estimate_flag = True
                # elif curr_drone_estimate_status != 0: # if we have an estimate
                #     no_estimate_flag = False
                
                # if time_start_of_no_estimate.secs - now.secs > 3:
                #     print("no estimate for 3 seconds!")
                #     pub_action.publish(Twist()) # send empty message to stop current ddpg action of drone
                #     use_ddpg = False # turn off ddpg
                #     break
                ...
                # rotate pos and vel vectors -yaw degrees about z in state array so the pos and vel are expressed in body coordinates
                state_reduced[0:3] = env.rotate_vector(state_reduced[0:3], -yaw) 
                state_reduced[3:6] = env.rotate_vector(state_reduced[3:6], -yaw) 
                state_reduced[6:9] = env.rotate_vector(state_reduced[6:9], -yaw)
                ...
                # choose action
                # print("actor_imported.predict(state_reduced)", actor_imported.predict(state_reduced))
