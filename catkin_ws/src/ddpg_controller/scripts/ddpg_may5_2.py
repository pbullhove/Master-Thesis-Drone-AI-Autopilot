...
class DroneState():
    ...
    def __init__(self):
        ...
        self.minx = self.x_goal - 2.0
        self.maxx = self.x_goal + 2.0
        self.miny = self.y_goal - 2.0
        self.maxy = self.y_goal + 2.0
        self.minz = self.z_goal - 1.0
        self.maxz = self.z_goal + 1.0
        self.minrotx = -20
        self.maxrotx = 20
        self.minroty = -20
        self.maxroty = 20
    ...
    def set_start_state(self):
        ...
        # if start_position[2] <= env.z_goal:
        #     start_position[2] = env.z_goal
    ...
    def step(self, state_reduced, action):
        ...
        reward, done, pos_error, delta_list = self.get_reward(action, next_state)

        return next_state, reward, done, pos_error, delta_list
    ...
    def get_reward(self, current_action, next_state):
        ...
        delta_list = [delta_x, delta_y, delta_z]

        # current_action = np.array(current_action)
        # factor_action = 0.2 * 0.0
        ...
        return reward, done, pos_error, delta_list
...
if train_agent == False:
    actor_imported = tf.keras.models.load_model('/home/danie/catkin_ws/src/ddpg/src/results/may5_2/actor_model.hdf5')
...
max_steps = 5000 # max number of steps in each training episode
...
def run_episode(max_steps, training):
    ...
    for step_number in range(max_steps):
        ...
        if training:
            ...
            action_clipped = np.clip(pure_action, -0.4, 0.4)
            ...
            if env.fsm_state == 1:
                print("HOVER")
                if prev_pos_error < 0.07:
                    # downscale by half
                    # print("DOWNSCALE BENCHOD")
                    action = 0.5 * action_clipped
                    if prev_pos_error < 0.03:
                        # downscale by a quarter
                        action = 0.25 * action_clipped
                        hovered_in_place_counter += 1
                        if hovered_in_place_counter >= 12: # if we have hovered for 12 step (4 sec given 3Hz controller)
                            # go to descend if hovering complete
                            env.fsm_state = 2 # finished hovering, start descending
                            hovered_in_place_counter = 0           
            elif env.fsm_state == 2:
                # print("DESCEND")
                # action = 0.25 * action_clipped
                # env.z_goal = 0.3
                ...
                #         env.fsm_state = 3
                print("DESCEND")
                # for i in range(len(prev_delta_list)):
                #     if prev_delta_list[i] < 0.1:
                #         action[i] = 0.25 * action_clipped[i]
                if prev_pos_error > 0.08:
                    action = 0.5 * action_clipped
                else:
                    action = 0.25 * action_clipped
                #update goal and bounds
                if prev_pos_error < 0.03:
                    hovered_in_place_counter += 1
                    if hovered_in_place_counter >= 12:
                        env.z_goal -= 0.3
                        ...
                        hovered_in_place_counter = 0
                        if env.z_goal <= 0.0:
                            env.fsm_state = 3 # if goal is low enough, we initiate landing
            if env.fsm_state == 3:
                print("LAND")
                land()
                env.fsm_state = 4
        ...
        print("action:", action)
        next_state, reward, done, pos_error, delta_list = env.step(state_reduced, action) # apply action to drone
